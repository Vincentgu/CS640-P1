...
class macTableClass(object):
	def __init__(self, mac1, port1, arrival_time1):
		self.mac = mac1
		self.port = port1
		self.arrival_time = arrival_time1 
...
def expiryTimerHandler():
	now = int(time.time())
	log_info("ExpiryTimerHandler at Time {}".format(now))
	...
	for macObj in macTable:
		if macObj.expiryTime <= now:
			log_info("#RG#  Delete this MAC Address {}".format(macObj.mac))
...
def switchy_main(net):
	...
	while True:
		try:
			iport,timestamp,packet = net.recv_packet(10, True)
		except NoPackets:
		...
		if packet[0].src != "ff:ff:ff:ff:ff:ff":
			for macObj in macTable:
				if packet[0].src == macObj.mac:
					log_info("Mac {} is already present in Global Mac Table".format(macObj.mac))
